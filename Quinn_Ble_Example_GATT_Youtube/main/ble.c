...
typedef void (*ble_data_recv_handle_t)(uint8_t *data, uint16_t len);
...
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param) {
    if (param->exec_write.exec_write_flag == ESP_GATT_PREP_WRITE_EXEC) {
        ble_data_recv_handle(prepare_write_env->prepare_buf, prepare_write_env->prepare_len);
    } else {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep) {
            ...
            if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2) {
                ...
                if (descr_value == 0x0001 && (a_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY)) {
                    ESP_LOGI(GATTS_TAG, "notify enable");
                } else if (descr_value == 0x0002 && (a_property & ESP_GATT_CHAR_PROP_BIT_INDICATE)) {
...
void app_ble_send_data(uint8_t *data, uint16_t len) {
    esp_ble_gatts_send_indicate(gl_profile_tab[PROFILE_A_APP_ID].gatts_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id,
                                gl_profile_tab[PROFILE_A_APP_ID].char_handle, len, data, false);
    }
